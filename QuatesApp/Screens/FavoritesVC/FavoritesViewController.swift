...
extension FavoritesViewController: UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        guard let cell = tableView.dequeueReusableCell(
            withIdentifier: Constants.quoteReuseIdentifier,
            for: indexPath
        ) as? QuoteCell else {
    ...
    private func cellData(for row: Int) -> (text: String, author: String) {
        ...
        case .joke:
            ...
            return (jokes[row].joke, Constants.authorRandomJoke)
        case .chucknorris:
            let chuckJokes = viewModel.chuckJokes
            return (chuckJokes[row].joke, Constants.authorChuckNorrisJoke)
        }
    }
}
...
extension FavoritesViewController: UITableViewDelegate {
    ...
    private func fetchHeaderElements(for section: SectionType) -> (image: UIImage, title: String) {
        switch section {
        case .quote:
            return (image: .quote, title: Constants.favoriteQuotes)
        case .joke:
            return (image: .joke, title: Constants.favoriteJokes)
        case .chucknorris:
            return (image: .chuck, title: Constants.favoriteChuckNorris)
        }
    }
...
extension FavoritesViewController {
    ...
    private func presentDeleteAlert(for id: String, at indexPath: IndexPath) {
        let alert = UIAlertController(title: Constants.favoriteDeleteMessage, message: Constants.favoriteDelete, preferredStyle: .alert)
        let actionDone = UIAlertAction(title: Constants.alertYes, style: .default) { [weak self] _ in
        ...
        let actionCancel = UIAlertAction(title: Constants.alertCancel, style: .cancel)
